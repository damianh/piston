...
public static class TestDetailRenderer
{
    ...
    private static string RenderTest(TestResultDto test)
    {
        ...
        {
            TestStatusDto.Passed  => "[green3]PASSED[/]",
            TestStatusDto.Failed  => "[red3]FAILED[/]",
            TestStatusDto.Skipped => "[gold1]SKIPPED[/]",
            TestStatusDto.Running => "[cyan]RUNNING[/]",
            _                     => "[dim]NOT RUN[/]",
        };
        sb.AppendLine($"Status:    {statusMarkup}");
        sb.AppendLine($"Duration:  [dim]{test.DurationMs:F0}ms[/]");
    ...
    private static string RenderSuite(TestSuiteDto suite)
    {
        var passed  = suite.Tests.Count(t => t.Status == TestStatusDto.Passed);
        var failed  = suite.Tests.Count(t => t.Status == TestStatusDto.Failed);
        var skipped = suite.Tests.Count(t => t.Status == TestStatusDto.Skipped);
        ...
        return $"[bold]{Escape(suite.Name)}[/]\n\n" +
               ...
               $"Duration: [dim]{suite.TotalDurationMs / 1000.0:F2}s[/]";
    }
